...
import de.anton.invoice.checker.invoice_checker.model.*;
...
public class AppController {
    ...
    private String path2examples = "./examples";
...
// lastDetectedInvoiceType wird nicht mehr benötigt, da CRUD-Panel eigene Auswahl hat
    ...
    public AppController(AnwendungsModell model, MainFrame view) {
        ...
        if (this.view == null || this.model == null)
            throw new IllegalArgumentException("Modell/View dürfen nicht null sein!");
        initializeListeners();
        ConfigurationDialog.setPath2examples(path2examples);
    }
    ...
    private void setupDateiAuswahlDialog() {
        dateiAuswahlDialog = new JFileChooser(path2examples);
    ...
    private void handleLadePdfAktion(ActionEvent e) {
        ...
        if (ret == JFileChooser.APPROVE_OPTION) {
            ...
            if (files != null && files.length > 0) {
                ...
                for (File f : files) pdfPaths.add(f.toPath());
                ...
                model.ladeUndVerarbeitePdfs(pdfPaths, pGui,
                        pDoc -> {
                            if (pDoc != null)
                                SwingUtilities.invokeLater(() -> view.logMessage("Verarbeitet:" + pDoc.getSourcePdf() + (pDoc.getError() != null ? " [F]" : "")));
                        },
                        prog -> {
                            SwingUtilities.invokeLater(() -> {
                                view.setProgressBarValue((int) (prog * 100));
                                if (prog >= 1.0) {
                                    Timer t = new Timer(2000, ae -> view.setProgressBarVisible(false));
                                    t.setRepeats(false);
                                    t.start();
                                }
                            });
                ...
                log.warn("Keine Dateien ausgewählt.");
                view.logMessage("Keine Dateien ausgewählt.");
            }
        } else {
            log.info("Dateiauswahl abgebrochen.");
            view.logMessage("Dateiauswahl abgebrochen.");
        }
    }
    ...
    private void handleExportExcelAktion(ActionEvent e) {
        ...
        if (view == null) return;
        dateiAuswahlDialog.setDialogTitle("Excel speichern");
        dateiAuswahlDialog.setMultiSelectionEnabled(false);
        dateiAuswahlDialog.setFileFilter(new FileNameExtensionFilter("Excel (*.xlsx)", "xlsx"));
        dateiAuswahlDialog.setSelectedFile(new File("Export.xlsx"));
        if (dateiAuswahlDialog.showSaveDialog(view) == JFileChooser.APPROVE_OPTION) {
            Path ziel = dateiAuswahlDialog.getSelectedFile().toPath();
            if (!ziel.toString().toLowerCase().endsWith(".xlsx"))
                ziel = ziel.resolveSibling(ziel.getFileName() + ".xlsx");
            final Path finalZiel = ziel;
            view.logMessage("Exportiere...");
            view.setProgressBarVisible(true);
            view.setProgressBarValue(0);
            new SwingWorker<Void, Void>() {
                ...
                protected Void doInBackground() {
                    try {
                        model.exportiereAlleNachExcel(finalZiel);
                        ok = true;
                    } catch (Exception ex) {
                        err = ex.getMessage();
                        log.error("Excel Fehler", ex);
                    } finally {
                        SwingUtilities.invokeLater(() -> {
                            view.setProgressBarValue(100);
                            new Timer(1000, ae -> view.setProgressBarVisible(false)).start();
                        });
                    }
                    return null;
                }
                ...
                @Override
                protected void done() {
                    if (ok) JOptionPane.showMessageDialog(view, "Export erfolgreich:\n" + finalZiel, "Erfolg", 1);
                    else JOptionPane.showMessageDialog(view, "Export fehlgeschlagen.\n" + err, "Fehler", 0);
                    view.logMessage(ok ? "Export fertig." : "Fehler Export.");
                }
            }.execute();
        } else {
            log.info("Export abgebrochen.");
            view.logMessage("Export abgebrochen.");
        }
    }
    ...
    private void handleFlavorChange(ActionEvent e) {
        if (view != null) view.logMessage("Flavor geändert. Klicke 'Neu verarbeiten' oder 'Speichern (CSV)'.");
    }

    private void handleRowTolChange(ChangeEvent e) {
        if (view != null && e.getSource() == view.getRowToleranceSpinner())
            view.logMessage("Row Tol geändert. Klicke 'Neu verarbeiten' oder 'Speichern (CSV)'.");
    }

    private void handleOpenConfigEditor(ActionEvent e) {
        log.info("Öffne Bereichs-Konfig-Editor...");
        if (view == null) return;
        ExtractionConfiguration cfg = model.getAktiveKonfiguration();
        ExtractionConfiguration cfgDlg;
        if (cfg != null) {
            cfgDlg = model.getConfigurationService().loadConfiguration(cfg.getName());
            if (cfgDlg == null) cfgDlg = new ExtractionConfiguration("Neu");
        } else cfgDlg = new ExtractionConfiguration("Neu");
        ConfigurationDialog dlg = new ConfigurationDialog(view, model.getConfigurationService(), cfgDlg);
        dlg.setVisible(true);
        ExtractionConfiguration saved = dlg.getSavedConfiguration();
        if (saved != null) {
            log.info("Bereichs-Konfig '{}' gespeichert.", saved.getName());
            updateAvailableConfigsInView();
            model.setAktiveKonfiguration(saved);
            view.logMessage("Konfig '" + saved.getName() + "' gespeichert/aktiviert. Ggf. 'Neu verarbeiten'.");
        } else {
            log.info("Dialog geschlossen ohne Speichern.");
            updateAvailableConfigsInView();
        }
    }
    ...
    private void handleConfigSelectionChange(ItemEvent e) {
        if (e.getStateChange() == ItemEvent.SELECTED) {
            ...
            ExtractionConfiguration cfg = null;
            if (item instanceof ExtractionConfiguration) cfg = (ExtractionConfiguration) item;
            else if (!"Keine".equals(item)) return;
            log.info("Bereichs-Konfig Auswahl: {}", (cfg != null ? cfg.getName() : "Keine"));
            if (!Objects.equals(model.getAktiveKonfiguration(), cfg)) {
                model.setAktiveKonfiguration(cfg);
                view.logMessage("Bereichs-Konfig auf '" + (cfg != null ? cfg.getName() : "Keine") + "' gesetzt. Ggf. 'Neu verarbeiten'.");
            }
        }
    }

    private void handleEditCsv(ActionEvent e) {
        log.info("Edit CSV geklickt.");
        model.getInvoiceTypeService().openCsvInEditor();
    }

    private void handleRefreshAction(ActionEvent e) {
        triggerReprocessing("Refresh Button");
    }
    ...
    private void triggerReprocessing(String grund) {
        ...
        if (selectedDoc != null && selectedDoc.getFullPath() != null && !selectedDoc.getFullPath().isBlank()) {
            ...
            try {
                ...
                log.error("Pfadfehler", pathEx);
                view.logMessage("Fehler: Ungültiger Pfad.");
                return;
            }
            if (pdfPath != null) {
                ...
                model.ladeUndVerarbeitePdfsMitKonfiguration(Collections.singletonList(pdfPath), aktuelleParameterGui, aktiveBereichsKonfig,
                        processedDoc -> { /* Status */
                            if (processedDoc != null)
                                SwingUtilities.invokeLater(() -> view.logMessage("Neu verarbeitet:" + processedDoc.getSourcePdf() + (processedDoc.getError() != null ? " [FEHLER]" : "")));
                        },
                        progress -> { /* Progress */
                            SwingUtilities.invokeLater(() -> {
            ...
            log.debug("Kein PDF ausgewählt.");
            view.logMessage("Bitte zuerst ein PDF auswählen.");
        }
    }
    ...
    private Map<String, String> getCurrentParametersFromGui() {
        ...
        try {
            ...
            log.error("Fehler GUI-Params", e);
            p.putIfAbsent("flavor", "lattice");
            p.putIfAbsent("row_tol", "2");
        }
        return p;
    }
    ...
    private void updateAvailableConfigsInView() {
        ...
        log.debug("Aktualisiere Bereichs-Konfig-ComboBox...");
    ...
    public void refreshInvoiceTypePanelForCurrentSelection() {
        ...
        if (selectedDoc != null) {
            ...
            if (configToShow == null) {
                log.warn("DetectedInvoiceType in Doc ist null!");
                configToShow = model.getInvoiceTypeService().getDefaultConfig();
            }
        } else {
            log.debug("Kein Dokument ausgewählt.");
        }
    ...
    public String getPath2examples() {
        return path2examples;
    }

    public void setPath2examples(String path2examples) {
        this.path2examples = path2examples;
    }
}